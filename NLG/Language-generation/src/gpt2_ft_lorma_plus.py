...
from model_lorma_plus import GPT2Config, GPT2LMModel
...
parser.add_argument('--mult_lora_dim', type=int, default=0, help='mult lora attn dimension')

parser.add_argument('--mult_lora_alpha', type=int, default=128, help='mult lora attn alpha')

parser.add_argument('--obj', default='clm', choices=['jlm', 'clm'], 
                    help='language model training objective')

parser.add_argument('--mult_lora_dropout', default=0.0, type=float, 
...
def mult_lora_state_dict(model, bias="none"):
    my_state_dict = model.state_dict()
    if bias == "none":
        return {k: my_state_dict[k] for k in my_state_dict if "lora" in k}
    elif bias == "all":
        return {k: my_state_dict[k] for k in my_state_dict if "lora" in k or "bias" in k}
    elif bias == "lora_only":
        to_return = {}
    ...
    for idx, data in enumerate(train_loader):
        ...
        if train_step % args.save_interval == 0: 
            if args.rank == 0:
                ...
                torch.save({'model_state_dict': mult_lora_state_dict(model)}, model_path)
...
if __name__ == '__main__':
    ...
    if args.model_card == 'gpt2.sm':
        config = GPT2Config(
            ...
            lora_attn_dim=args.mult_lora_dim, 
            lora_attn_alpha=args.mult_lora_alpha, 
            lora_dropout=args.mult_lora_dropout,
        )
    ...
    if args.mult_lora_dim > 0:
        mark_only_lora_as_trainable(lm_net)
    ...
    trainable_params = sum(p.numel() for p in lm_net.parameters() if p.requires_grad)
