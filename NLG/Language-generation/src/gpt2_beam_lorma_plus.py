...
def beam(model, data_iter, args):
    ...
    print(f"data_iter length: {len(data_iter)}")
...
if __name__ == '__main__':
    ...
    if args.model_card == 'gpt2.sm':
        config = GPT2Config(
            ...
            lora_attn_dim=args.mult_lora_dim, lora_attn_alpha=args.mult_lora_alpha,
        )
    ...
    if args.init_checkpoint is not None:
        ...
        cp = torch.load(args.init_checkpoint, map_location=torch.device(args.device))
