...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False):
        ...
        self.config = config
        # ----- Mult-LoRA params -----
        self.mult_lora_r = config.lora_attn_dim # the name of the param in GPT2Config has not changed
        if self.mult_lora_r > 0:
            self.mult_lora_alpha = config.lora_attn_alpha
            self.mult_lora_scaling = self.mult_lora_alpha / self.mult_lora_r
            
            # config.enable_lora = [True, False, True]
            # self.lora_A_q = nn.Parameter(torch.zeros(self.mult_lora_r, n_state))
            self.lora_A_q = nn.Parameter(self.c_attn.weight.new_zeros((self.mult_lora_r, n_state)))
            self.lora_B_q = nn.Parameter(self.c_attn.weight.new_zeros((n_state, self.mult_lora_r)))
            self.lora_A_v = nn.Parameter(self.c_attn.weight.new_zeros((self.mult_lora_r, n_state)))
            self.lora_B_v = nn.Parameter(self.c_attn.weight.new_zeros((n_state, self.mult_lora_r)))
            
            # init lora_params
            # SHIFT INIT !!!
            with torch.no_grad():
                nn.init.kaiming_uniform_(self.lora_B_q, a=math.sqrt(5))
                self.lora_B_q.data[:, 0] = 1.0
                self.lora_A_q.data.zero_()
                self.lora_A_q.data[0,0] = 1.0
                self.lora_A_q.data /= math.sqrt(self.mult_lora_scaling)
                self.lora_B_q.data /= math.sqrt(self.mult_lora_scaling)
                
                nn.init.kaiming_uniform_(self.lora_B_v, a=math.sqrt(5))
                self.lora_B_v.data[:, 0] = 1.0
                self.lora_A_v.data.zero_()
                self.lora_A_v.data[0,0] = 1.0
                self.lora_A_v.data /= math.sqrt(self.mult_lora_scaling)
                self.lora_B_v.data /= math.sqrt(self.mult_lora_scaling)
    ...
    def forward(self, x, history=None, layer_past=None, len_past=None):
        ...
        n_state = self.split_size
        
        if self.mult_lora_r > 0:
            # Mult-LoRA forward
            weight = self.c_attn.weight # [nx, 3*n_state]
            bias = self.c_attn.bias # [3*n_state,]
            # slice out the query, key and value weights
            W_q_raw = weight[:, :n_state] # [nx, n_state]
            b_q = bias[:n_state] if bias is not None else None
            W_k_raw = weight[:, n_state:2*n_state]
            b_k = bias[n_state:2*n_state] if bias is not None else None
            W_v_raw = weight[:, 2*n_state:3*n_state]
            b_v = bias[2*n_state:3*n_state] if bias is not None else None
            
            # transpose to [n_state, nx] (conventional F.linear weights)
            W_q = W_q_raw.t()
            W_k = W_k_raw.t()
            W_v = W_v_raw.t()
            
            # adapted_W_q = W_q + self.mult_lora_scaling * (self.lora_B_q @ (self.lora_A_q @ W_q))
            # adapted_W_v = W_v + self.mult_lora_scaling * (self.lora_B_v @ (self.lora_A_v @ W_v))
            
            adapted_W_q = (self.shift(self.lora_B_q @ self.lora_A_q) @ W_q) * self.mult_lora_scaling
            adapted_W_v = (self.shift(self.lora_B_v @ self.lora_A_v) @ W_v) * self.mult_lora_scaling
            
            query = F.linear(x, adapted_W_q, b_q)
            key = F.linear(x, W_k, b_k)
            value = F.linear(x, adapted_W_v, b_v)
        else:
            # standard path
            x = self.c_attn(x)
