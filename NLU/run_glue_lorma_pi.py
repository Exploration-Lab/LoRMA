...
class ModelArguments:
    ...
    apply_lorma: Optional[bool] = field(
        default=True,
        metadata={"help": "Apply LoRMA to the model."}
    )
    ...
    lorma_mode: Optional[str] = field(
        default="pre",
        metadata={"help": "`Mode to apply LoRMA pre`, `post`, `complete_post`, `complete_pre`."}
    )
    do_pi: Optional[bool] = field(
        default=False,
        metadata={"help": "to apply shift for Rank inflation or not."}
    )
    ...
    adapted_weights: Optional[str] = field(
        default="qv",
        metadata={"help": "Adapted weights for LoRMA_pi"}
    )
    ##
    ...
    def __post_init__(self):
        if self.apply_lorma == True:
            if self.lorma_mode not in ["pre", "post", "complete_post", "complete_pre"]:
                raise ValueError(f"Unknown mode: {self.lorma_mode}. Choose from ['pre', 'post', 'complete_post', 'complete_pre'].")



class LormaLinear_pi(nn.Module):
    """Custom Linear Layer with LoRMA integrations"""

    def __init__(self, original_linear: nn.Linear, lorma_r: int, lorma_alpha: int, mode: str, do_pi: bool = False, lorma_dropout: float = 0.0):
        super().__init__()
        
        # logger.info("*"*100)
        # logger.info(f"lorma_mode: {mode} lorma_r: {lorma_r} lorma_alpha {lorma_alpha} do_pi: {do_pi}")
        # logger.info("*"*100)
        # sys.exit(0)
        
        assert mode in ["pre", "post", "complete_post", "complete_pre"], "mode must be either pre, post, complete_pre or complete_post"
        self.mode = mode
        ...
        # This is assuming that the application is for fan_in_fan_out = False layers (unlike GPT-2)
        ...
        elif self.mode == "complete_post":
            self.lora_A = nn.Parameter(original_linear.weight.new_zeros(self.in_features, self.in_features))
        else:
            # "complete_pre"
            self.lora_A = nn.Parameter(original_linear.weight.new_zeros(self.out_features, self.out_features))
        
        if (self.mode == "complete_post") or (self.mode == "complete_pre"):
            ...
            if not self.do_pi:
                ...
                # different initialization required for shift so that full rank
                with torch.no_grad():
                    # self.lora_B.data.zero_()
                    nn.init.kaiming_uniform_(self.lora_B, a=math.sqrt(5))
                    self.lora_B[:, 0] = 1.0
                    self.lora_A.data.zero_()
                    self.lora_A.data[0, 0] = 1.0
                    self.lora_A.data /= math.sqrt(self.scaling)
                    self.lora_B.data /= math.sqrt(self.scaling)
        
        # storing shift indices
        if self.do_pi:
    ...
    def forward(self, x):
        # logger.info("Called LormaLinear_pi forward")
        if self.mode == "pre":
            if self.do_pi:
                adapted_weight = self.shift(self.lora_B @ self.lora_A) @ self.original_layer.weight
            else:
                adapted_weight = self.lora_B @ (self.lora_A @ self.original_layer.weight)
            output = F.linear(self.lora_dropout(x), adapted_weight * self.scaling, self.original_layer.bias)
        elif self.mode == "post":
            # mode is "post"
            if self.do_pi:
                adapted_weight = self.original_layer.weight @ self.shift(self.lora_B @ self.lora_A)
            else:
                adapted_weight = (self.original_layer.weight @ self.lora_B) @ self.lora_A
            output = F.linear(self.lora_dropout(x), adapted_weight * self.scaling, self.original_layer.bias)
        elif self.mode == "complete_post":
            adapted_weight = (self.original_layer.weight @ self.lora_A)
            output = F.linear(self.lora_dropout(x), adapted_weight, self.original_layer.bias)
        else:
            # mode is complete_pre
            adapted_weight = (self.lora_A @ self.original_layer.weight)
            output = F.linear(self.lora_dropout(x), adapted_weight, self.original_layer.bias)
...
def main():
    ...
    # logger.info(f"model_args.apply_ft: {model_args.apply_ft}")
    
    # MANUAL MODIFYING THE MODEL
    ...
    # Marking the classifier layer as trainable
    for n, p in model.named_parameters():
        if "classifier" in n:
            p.requires_grad = True
